...
for(z=0;z<31;z++){d3.select("path#_"+data.results[0].data[z].row[2]).style("fill",colour_scale(z)).style("stroke-width","1px").attr("class","selected");var name=data.results[0].data[z].row[0];var value=data.results[0].data[z].row[1];value=value.toFixed(3); var rank = z +1;var output= rank + ": " + name+" - "+value;console.log(output);var div=document.getElementById('paragraph_text');div.innerHTML=div.innerHTML+output+'<br>';};},failure:function(msg){console.log("failed")}});}else{window.alert("NOT IN DATASET. Please choose a shaded country.");}})
