...
var projection = d3.geo.mercator().center([0, 35]).scale(200).rotate([0, 0, 0]);
...
function main(error, world, countryData) {
    ...
    countryData.forEach(function(d) {
        ...
        if (d.c1 > 0) {
            country_IDs[d.id] = d.id;
        }
        if (d.c1 > 0) {
            c1[d.id] = d.c1;
        }
        if (d.c1 > 0) {
            recipes[d.id] = d.recipes;
        }
        if (d.c1 > 0) {
            Ing[d.id] = d.Ing;
        }
    ...
    g.selectAll("path").data(countries)
        ...
        .on("mouseover", function(d) {
            country_selector.text(country_names[d.id])
                ...
                .style("display", "block").style("opacity", 0.8);
        ...
        .on("click", function(d) {
            if (c1[d.id] > -1) {
                ...
                var cypher = {
                    "statements": [{
                        "statement": "MATCH (a)-[n:SIMILIARITY]->(b) " + "WHERE a.name= " + '"' + country_selected + '"' 
                        + " AND b.scope = " + '"Country" ' + "RETURN b.name AS CUISINE, 1/n.value AS SIMILARITY, b.country_ID AS IDC " + "ORDER BY n.value DESC"
                    }]
                };
                $.ajax({
                    type: "POST",
                    ...
                    headers: {
                        "Authorization": "Basic <cmVjaXBlc19kYXRhYmFzZV9zaW1pbGFpcml0ZXM6ZGJSRnRScXJmMWg1VFpjbHdlMGY=>"
                    },
                    ...
                    success: function(data, textStatus, jqXHR) {
                        ...
                        if (recipes[d.id] > 1000)
                        	var stats = "Cuisine selected: " + country_names[d.id] + '<br>' + "Number of recipes: " + '<br>' + recipes[d.id] + '<br>' + 
                                        "Number of categories: " + Cat[d.id] + '<br>' + 
                                        "Number of ingredients: " + Ing[d.id] + '<br>'
                        else
                        	var stats = "Cuisine selected: " + country_names[d.id] + '<br>' + "Number of recipes: " + recipes[d.id] + '<br>' + 
                                        "Number of categories: " + Cat[d.id] + '<br>' + "Number of ingredients: " + Ing[d.id] + '<br>'
                        var div = document.getElementById('paragraph_text');
                        div.innerHTML = div.innerHTML + stats + "________________" + '<br>' ;
                        for (z = 0; z < 31; z++) {
                ...
                window.alert("NOT IN DATASET. Please choose a shaded country.");
            }
        })
...
var zooming = d3.behavior.zoom().on("zoom", function() {
